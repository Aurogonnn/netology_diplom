...
$sectionNameToRemove = "Распродажа";

foreach ($arResult["SECTIONS"] as $key => $section) {
    if ($section['NAME'] == $sectionNameToRemove) {
...
$i=0;
$count_items = count($arResult["SECTIONS"]);
$last_item = $count_items - 1;

foreach ($arResult['SECTIONS'] as $key => $section) {
    if (isset($section['RELATIVE_DEPTH_LEVEL']) && $section['RELATIVE_DEPTH_LEVEL'] == 2) {
        continue; // Пропускаем вложенные секции
    }
    if($i == 2) {
        $arResult['SECTIONS'][$key]['THIRD_ITEM'] = 'Y';
    } else {
        $arResult['SECTIONS'][$key]['THIRD_ITEM'] = 'N';
    }
    $i++;
}
...
if (!empty($arResult['SECTIONS'])) {
    $lastKey = array_key_last($arResult['SECTIONS']);
    $arResult['SECTIONS'][$lastKey]['LAST_ITEM'] = 'Y';
}
